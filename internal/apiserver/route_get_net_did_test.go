...
func TestGetNetIdentityByDID(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/api/v1/network/identities/did:gdc:org/org_1", nil)
...
func TestGetNetIdentityByDIDWithVerifiers(t *testing.T) {
	...
	nmn := &networkmapmocks.Manager{}
	o.On("NetworkMap").Return(nmn)
	req := httptest.NewRequest("GET", "/api/v1/network/identities/did:gdc:org/org_1?fetchverifiers", nil)
	req.Header.Set("Content-Type", "application/json; charset=utf-8")
	res := httptest.NewRecorder()
